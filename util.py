...
def read_int_bool_int(data, offset):
    return INT_BOOL_INT.unpack_from(data, offset), offset + INT_BOOL_INT.size


def read_int_long_int_long(data, offset):
    return INT_LONG_INT_LONG_STRUCT.unpack_from(data, offset), offset + INT_LONG_INT_LONG_STRUCT.size
...
class QuorumConfig(object):
    ...
    class Server(ConfigEntry):
        ...
        def __init__(self, sid, address_str):
            ...
            if len(server_client_parts) > 2 or len(server_parts) < 3 or len(server_parts) > 4:
